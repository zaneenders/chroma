...
struct FontAtlas {
  ...
  var atlasWidth: Int { charCount * Int(glyphWidth + glyphSpacing) }
  var atlasHeight: Int { Int(glyphHeight) }

  init(device: MTLDevice) {
    let font5x7 = Self.buildFont()
    // Compute atlas dimensions from defaults (can't use computed properties before self is ready)
    let w = (Int(lastChar) - Int(firstChar) + 1) * (Int(glyphWidth) + Int(glyphSpacing))
    let h = Int(glyphHeight)
    var pixels = [UInt8](repeating: 0, count: w * h * 4)

    for c in Int(firstChar)...Int(lastChar) {
      let g = font5x7[c]
      let xoff = (c - Int(firstChar)) * Int(glyphWidth + glyphSpacing)
      if !g.rows[0].isEmpty {
        for y in 0..<Int(glyphHeight) {
          let row = Array(g.rows[y])
          for x in 0..<Int(glyphWidth) {
            let bit = row[x] == "1"
            let idx = (y * w + xoff + x) * 4
            pixels[idx + 0] = 255
            pixels[idx + 1] = 255
            pixels[idx + 2] = 255
            pixels[idx + 3] = bit ? 255 : 0
          }
        }
      }
    }
  ...
  func glyphUV(_ c: UInt8) -> (Float, Float, Float, Float) {
    var ch = c
    if ch < firstChar || ch > lastChar { ch = 32 }
    let idx = Int(ch - firstChar)
    let xoff = idx * (Int(glyphWidth) + Int(glyphSpacing))
    let u0 = Float(xoff) / Float(atlasWidth)
    let u1 = Float(xoff + Int(glyphWidth)) / Float(atlasWidth)
    let v0 = Float(0)
    let v1 = Float(glyphHeight) / Float(atlasHeight)
    return (u0, v0, u1, v1)
  }

  // MARK: - 5×7 ASCII bitmap font

  private static func buildFont() -> [Glyph] {
    var font = Array(repeating: Glyph(), count: 128)
    func set(_ ch: Character, _ rows: [String]) {
      let i = Int(ch.unicodeScalars.first!.value)
      font[i] = Glyph(rows: rows)
    }
